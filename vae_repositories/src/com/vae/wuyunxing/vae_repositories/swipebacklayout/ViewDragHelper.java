...
package com.vae.wuyunxing.vae_repositories.swipebacklayout;
...
public class ViewDragHelper {
	...
	public static abstract class Callback {
		 ...
		 * values may be clamped to system minimums or maximums. <p> Calling code may decide to fling or otherwise release the view to let
		 * it settle into place. It should do so using {@link #settleCapturedViewAt(int, int)} or {@link #flingCapturedView(int, int, int,
		 * int)}. If the Callback invokes one of these methods, the ViewDragHelper will enter {@link #STATE_SETTLING} and the view capture
		 * will not fully end until it comes to a complete stop. If neither of these methods is invoked before <code>onViewReleased</code>
		 * returns, the view will stop in place and the ViewDragHelper will return to {@link #STATE_IDLE}. </p>
		 *
		 ...
		 * The callback should return true if the user is permitted to drag the given view with the indicated pointer. <p> ViewDragHelper
		 * may call this method multiple times for the same view even if the view is already captured; this indicates that a new pointer is
		 * trying to take control of the view. </p> <p> If this method returns true, a call to {@link #onViewCaptured(View, int)} will
	...
	/**
	 * Set the max velocity that will be detected as having a magnitude greater than zero in pixels per second. Callback methods accepting a
	 ...
	 * int)} and {@link me.imid.swipebacklayout.lib.ViewDragHelper.Callback#onEdgeDragStarted(int, int)} methods will only be invoked for
	 * edges for which edge tracking has been enabled.
	 *
	...
	/**
	 * Set the size of an edge. This is the range in pixels along the edges of this view that will actively detect edge touches or drags if
	 * edge tracking is enabled.
	 *
	...
	/**
	 * Capture a specific child view for dragging within the parent. The callback will be notified but {@link
	 * me.imid.swipebacklayout.lib.ViewDragHelper.Callback#tryCaptureView(View, int)} will not be asked permission to capture this view.
	 *
	 ...
	 * #continueSettling(boolean)} on each subsequent frame to continue the motion until it returns false. If this method returns false
	 * there is no further work to do to complete the movement. <p> This operation does not count as a capture event, though {@link
	 * #getCapturedView()} will still report the sliding view while the slide is in progress. </p>
	 *
	 ...
	 *                       from {@link View#computeScroll()} or similar methods invoked as part of layout or drawing.
	...
	/**
	 * Check if the given pointer ID represents a pointer that is currently down (to the best of the ViewDragHelper's knowledge). <p> The
	 * state used to report this information is populated by the methods {@link #shouldInterceptTouchEvent(MotionEvent)} or {@link
	 * #processTouchEvent(MotionEvent)}. If one of these methods has not been called for all relevant MotionEvents to track, the information
	...
	/**
	 * Check if any pointer tracked in the current gesture has crossed the required slop threshold. <p> This depends on internal state
	 * populated by {@link #shouldInterceptTouchEvent(MotionEvent)} or {@link #processTouchEvent(MotionEvent)}. You should only rely on the
	 * results of this method after all currently available touch data has been provided to one of these two methods. </p>
	 *
	...
	/**
	 * Check if the specified pointer tracked in the current gesture has crossed the required slop threshold. <p> This depends on internal
	 * state populated by {@link #shouldInterceptTouchEvent(MotionEvent)} or {@link #processTouchEvent(MotionEvent)}. You should only rely
	 * on the results of this method after all currently available touch data has been provided to one of these two methods. </p>
	 *
	 * @param directions Combination of direction flags, see {@link #DIRECTION_HORIZONTAL}, {@link #DIRECTION_VERTICAL}, {@link
	 *                   #DIRECTION_ALL}
